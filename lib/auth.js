...
// يُستخدم من المسارات العادية (Bearer header) ومن ترقية WebSocket (توكن في
// رابط الاتصال، لأن متصفحات الويب لا تسمح بإرسال هيدرز مخصّصة مع WebSocket)
...
export function requireAuth(req, res, next) {
  ...
  if (!user) return res.status(401).json({ ok: false, error: "الجلسة غير صالحة أو منتهية" });
