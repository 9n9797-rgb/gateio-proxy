...
const pairMetaCache = new Map(); // pair -> { meta, ts }
const PAIR_META_CACHE_MS = 10 * 60 * 1000;

// بيانات الزوج الرسمية من Gate.io: trade_status (موقوف؟)، st_tag (عملة عليها
// "إشعار خاص" — علامة Gate.io نفسها للعملات عالية الخطورة)، وfee (نسبة عمولة
// التداول الفعلية على هذا الزوج تحديداً — بعض العملات الجديدة/الخطرة قد
// تكون عمولتها أعلى من المعتاد 0.2%).
async function fetchPairMeta(pair) {
  const cached = pairMetaCache.get(pair);
  if (cached && Date.now() - cached.ts < PAIR_META_CACHE_MS) return cached.meta;
  try {
    const r = await fetch(`https://api.gateio.ws/api/v4/spot/currency_pairs/${pair}`);
    if (!r.ok) return null;
    const meta = await r.json();
    pairMetaCache.set(pair, { meta, ts: Date.now() });
    return meta;
  } catch (_) {
    return null;
  }
...
export function detectFeeRisk(meta) {
  if (!meta) return { risk: false };
  const fee = Number(meta.fee || 0);
  const notTradable = meta.trade_status && meta.trade_status !== "tradable";
  ...
  if (notTradable) {
    return {
      ...
      severity: "extreme",
      fee_pct: fee,
      reason: `⚠️ هذا الزوج موقوف عن التداول حالياً على Gate.io (${meta.trade_status})`
    };
  }
  if (meta.delisting_time) {
    const date = new Date(meta.delisting_time * 1000).toLocaleDateString("ar");
    return {
      risk: true,
      severity: "extreme",
      fee_pct: fee,
      reason: `⚠️ Gate.io جدول هذه العملة للحذف من التداول (Delisting) بتاريخ ${date} — لا تفتح صفقات جديدة عليها`
    };
  }
  if (meta.st_tag) {
    return {
      ...
      severity: "high",
      fee_pct: fee,
      reason: "⚠️ Gate.io وضع \"إشعار خاص\" (ST) على هذه العملة — علامة رسمية من المنصة بارتفاع مخاطرها (تلاعب/سيولة/إفلاس مشروع)، يُنصح بتجنّبها"
    };
  }
  if (fee >= 1) {
    return {
      risk: true,
      severity: fee >= 5 ? "extreme" : "high",
      fee_pct: fee,
      reason: `⚠️ عمولة التداول على هذا الزوج تحديداً ${fee}% (أعلى من المعتاد 0.2%) — تلتهم الربح حتى لو نجحت الصفقة`
    };
  }
  return { risk: false, fee_pct: fee };
}

// يُجمَع تقييم البمب والسبريد والعمولة/الحالة الرسمية لزوج معيّن — يُستخدم في
// autopilot.js قبل تنفيذ أي شراء (تطبيق موحّد على كل المستخدمين لأن البيانات
// سوقية وليست شخصية).
export async function evaluateMarketRisk(pair) {
  const [dailyRaw, ticker, meta] = await Promise.all([
    fetchDailyCandles(pair),
  ...
  const feeRisk = detectFeeRisk(meta);

  const severityRank = { medium: 1, high: 2, extreme: 3 };
  let worst = null;
  for (const candidate of [pump, spread, feeRisk]) {
    if (candidate.risk && (!worst || (severityRank[candidate.severity] || 0) > (severityRank[worst.severity] || 0))) {
      worst = candidate;
    }
  }

  return { pump, spread, fee: feeRisk, worst: worst || { risk: false } };
}
