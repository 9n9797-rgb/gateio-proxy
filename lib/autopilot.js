...
import { evaluateMarketRisk } from "./marketRisk.js";
...
// يطبَّق على كل المستخدمين بنفس الطريقة لأنه يعتمد على بيانات سوق عامة
// (بمب أند دمب + فرق سعر العرض/الطلب)، لا على تاريخ صفقات مستخدم معيّن.
// مخاطر شديدة/مرتفعة تُلغي إشارة الشراء كلياً (تتحول إلى HOLD)، والمتوسطة
// تُبقي الإشارة لكن مع تحذير واضح يُعرض للمستخدم.
function applyMarketRisk(decision, marketRisk) {
  decision.market_risk = marketRisk.worst.risk ? marketRisk.worst : null;
  if (decision.action === "BUY" && marketRisk.worst.risk) {
    if (marketRisk.worst.severity === "extreme" || marketRisk.worst.severity === "high") {
      decision.action = "HOLD";
      decision.market_risk_blocked = true;
    }
  }
  return decision;
}
...
async function processUserPair(user, settings, pair) {
  ...
  const quantDecision = decide(user.id, pair, raw);
  const decision = await refineDecision(user.id, pair, quantDecision);
  if (decision.action === "BUY") {
    applyMarketRisk(decision, await evaluateMarketRisk(pair));
  }
...
export async function executeRecommendation(userId, recommendationId) {
  ...
  if (rec.action === "BUY") {
    ...
    // إعادة فحص خطر السوق وقت التنفيذ الفعلي (قد يكون تغيّر منذ إنشاء التوصية)
    const marketRisk = await evaluateMarketRisk(rec.pair);
    if (marketRisk.worst.risk && (marketRisk.worst.severity === "extreme" || marketRisk.worst.severity === "high")) {
      db.prepare("UPDATE recommendations SET status = 'dismissed' WHERE id = ?").run(recommendationId);
      throw new Error(marketRisk.worst.reason);
    }
...
export async function analyzePair(userId, pair, interval = "15m") {
  ...
  const quantDecision = decide(userId, pair, raw);
