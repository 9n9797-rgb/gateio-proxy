...
function recentMistakesSummary(userId, pair, limit = 20) {
  const losses = db
    .prepare(
      "SELECT * FROM trades WHERE user_id = ? AND pair = ? AND pnl_pct < 0 ORDER BY id DESC LIMIT ?"
    )
    .all(userId, pair, limit);
  if (losses.length === 0) return "لا توجد صفقات خاسرة سابقة مسجّلة لهذا الزوج.";

  const avgLoss = losses.reduce((a, t) => a + t.pnl_pct, 0) / losses.length;
  const commonVotes = {};
  for (const t of losses) {
    const votes = t.votes ? JSON.parse(t.votes) : {};
...
export async function refineDecision(userId, pair, quantDecision) {
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) return { ...quantDecision, engine: "quant", ai_reason: null };

  const model = process.env.OPENAI_MODEL || "gpt-4o-mini";
  const mistakes = recentMistakesSummary(userId, pair);

  const mistakeNote = quantDecision.mistake_risk > 0.7
    ? `تحذير: المحرك الكمّي خفّف الدرجة الأصلية (${quantDecision.raw_score}) إلى (${quantDecision.score}) لأن نمط الأصوات الحالي يشبه بنسبة ${Math.round(quantDecision.mistake_risk * 100)}% أنماطاً تسبّبت بخسائر سابقة لهذا المستخدم على هذا الزوج (${quantDecision.mistake_matches} حالة مشابهة).`
    : "";
...
${mistakeNote}
هل تؤكد هذا القرار أم تعدّله؟ كن أكثر تحفّظاً إن وُجد تحذير تشابه مع أخطاء سابقة. أجب بصيغة JSON فقط بالشكل:
