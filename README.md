...
لا توجد كلمة مرور أدمن منفصلة — الدخول يتم بنفس نظام الرمز المرسل للبريد
(تسجيل الدخول نفسه هو آلية "استعادة الوصول": لا كلمة مرور لتُفقد، فقط رمز
جديد يُرسل كل مرة). الصلاحية تُمنح فقط لبريد مذكور في متغير البيئة
`ADMIN_EMAILS` (قيم مفصولة بفواصل، مثل `a@x.com,b@y.com`). أي حساب آخر يحاول
الوصول لمسارات `/admin/*` يحصل على 403.
