...
- **عمولة/حالة الزوج الرسمية على Gate.io**: زوج موقوف عن التداول، أو مجدول
  للحذف (Delisting)، أو عليه علامة "إشعار خاص" (ST — علامة Gate.io الرسمية
  للعملات عالية الخطورة)، أو عمولة تداول أعلى من المعتاد (٠.٢٪) على هذا
  الزوج تحديداً — كل هذه تُحجَب فوراً قبل الدخول.
...
`auto` — ويُعاد فحصها مجدداً وقت تنفيذ أي توصية معلّقة (وليس فقط وقت إنشائها)
لأن حالة السوق قد تتغيّر بين الإنشاء والتنفيذ. مخاطر `medium` تُبقي الإشارة
لكن مع تحذير واضح (`market_risk`) يظهر في `GET /autopilot/analyze/:pair`
وصفحة `/app/analyze.html`.
